<?php
session_start();
include '../includes/db_connect.php';

// Check if user is logged in and is a Waiter
if (!isset($_SESSION['staff_id']) || $_SESSION['staff_role'] !== 'Waiter') {
    header("Location: staff_login.php");
    exit();
}

$staff_id = $_SESSION['staff_id'];
$staff_name = $_SESSION['staff_name'];

// Fetch ready orders (both cooking and beverage) to be served
$orders_sql = "SELECT DISTINCT o.order_id, o.table_id, o.total_amount, o.created_at,
               GROUP_CONCAT(CONCAT(m.name, ' x', oi.quantity) SEPARATOR ', ') as items,
               GROUP_CONCAT(DISTINCT m.item_type SEPARATOR ', ') as types
               FROM orders o
               JOIN order_items oi ON o.order_id = oi.order_id
               JOIN menu_items m ON oi.item_id = m.item_id
               WHERE o.order_status = 'Ready'
               GROUP BY o.order_id
               ORDER BY o.created_at ASC";
$orders_result = $conn->query($orders_sql);

// Mark order as served when waiter confirms delivery
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['order_id'])) {
    $order_id = $_POST['order_id'];
    
    $update_sql = "UPDATE orders SET order_status = 'Served' WHERE order_id = $order_id";
    if ($conn->query($update_sql)) {
        $_SESSION['success'] = "Order #$order_id delivered!";
    }
    header("Location: waiter_dashboard.php");
    exit();
}

// Get total ready orders
$total_sql = "SELECT COUNT(DISTINCT o.order_id) as total FROM orders o
              WHERE o.order_status = 'Ready'";
$total_result = $conn->query($total_sql);
$total_row = $total_result->fetch_assoc();
$ready_count = $total_row['total'];
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Waiter Dashboard | P&S Cafe</title>
    <link rel="stylesheet" href="../admin/admin_styles.css">
    <style>
        body {
            font-family: 'Manrope', sans-serif;
            background: #1a1a1a;
            color: #000;
        }
        
        .header {
            background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%);
            color: white;
            border-bottom: 3px solid #FFD700;
            padding: 20px;
            text-align: center;
            box-shadow: 0 4px 15px rgba(255, 215, 0, 0.1);
        }
        .header h1 { 
            font-size: 1.8em; 
            margin-bottom: 5px; 
            color: #FFD700;
            font-weight: 700;
        }
        .header-info { 
            font-size: 0.95em; 
            opacity: 0.9;
            color: #ffffff;
        }
        
        .container { max-width: 1200px; margin: 20px auto; padding: 0 15px; }
        
        .stats-bar {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
            gap: 15px;
            margin-bottom: 25px;
        }
        .stat-card {
            background: white;
            padding: 20px;
            border-radius: 10px;
            box-shadow: 0 2px 8px rgba(0,0,0,0.1);
            text-align: center;
        }
        .stat-card h3 { font-size: 2em; color: #2196f3; margin-bottom: 5px; }
        .stat-card p { color: #666; font-size: 0.95em; }
        
        .orders-grid {
            display: grid;
            grid-template-columns: repeat(auto-fill, minmax(350px, 1fr));
            gap: 20px;
        }
        
        .order-card {
            background: white;
            border-left: 5px solid #4caf50;
            padding: 20px;
            border-radius: 8px;
            box-shadow: 0 2px 8px rgba(0,0,0,0.1);
        }
        .order-card.urgent {
            border-left-color: #ff9800;
        }
        
        .order-header {
            display: flex;
            justify-content: space-between;
            align-items: center;
            margin-bottom: 12px;
            border-bottom: 2px solid #f0f0f0;
            padding-bottom: 10px;
        }
        .order-id { font-size: 1.3em; font-weight: bold; color: #333; }
        .table-no { background: #2196f3; color: white; padding: 5px 10px; border-radius: 5px; font-weight: bold; }
        
        .order-items {
            margin: 15px 0;
            padding: 10px;
            background: #fafafa;
            border-left: 3px solid #2196f3;
            border-radius: 4px;
        }
        .order-items h4 { color: #333; margin-bottom: 8px; font-size: 0.9em; }
        .items-list { font-size: 0.95em; line-height: 1.6; color: #555; }
        .item-type { display: inline-block; background: #e3f2fd; color: #1976d2; padding: 2px 6px; border-radius: 3px; font-size: 0.8em; margin-right: 5px; }
        
        .order-meta {
            display: flex;
            gap: 15px;
            margin: 12px 0;
            font-size: 0.9em;
        }
        .meta-item { flex: 1; }
        .meta-label { color: #999; font-size: 0.85em; }
        .meta-value { color: #333; font-weight: bold; }
        
        .status-badge {
            display: inline-block;
            padding: 6px 12px;
            border-radius: 20px;
            font-size: 0.85em;
            font-weight: bold;
            margin-top: 10px;
            background: #c8e6c9;
            color: #2e7d32;
        }
        
        .action-buttons {
            display: flex;
            gap: 10px;
            margin-top: 15px;
        }
        .btn {
            flex: 1;
            padding: 12px;
            border: none;
            border-radius: 6px;
            font-weight: bold;
            cursor: pointer;
            text-decoration: none;
            text-align: center;
            transition: all 0.3s;
            font-size: 0.95em;
            background: #4caf50;
            color: white;
        }
        .btn:hover {
            background: #388e3c;
            transform: translateY(-2px);
            box-shadow: 0 4px 8px rgba(0,0,0,0.2);
        }
        
        .empty-state {
            text-align: center;
            padding: 50px 20px;
            color: #999;
        }
        .empty-state h2 { font-size: 1.8em; margin-bottom: 10px; }
        
        .logout-btn {
            position: absolute;
            top: 20px;
            right: 20px;
            background: white;
            color: #2196f3;
            border: 2px solid #2196f3;
            padding: 8px 15px;
            border-radius: 6px;
            text-decoration: none;
            font-weight: bold;
            cursor: pointer;
        }
        .logout-btn:hover { background: #2196f3; color: white; }
    </style>
</head>
<body data-staff-role="Waiter">
    <div class="header">
        <div class="logout-btn" onclick="if(confirm('Logout?')) window.location='staff_login.php'">Logout</div>
        <h1>🚶 Waiter Dashboard</h1>
        <div class="header-info">Welcome, <strong><?php echo htmlspecialchars($staff_name); ?></strong></div>
    </div>

    <div class="container">
        <div class="stats-bar">
            <div class="stat-card">
                <h3><?php echo $ready_count; ?></h3>
                <p>Ready to Serve</p>
            </div>
            <div class="stat-card">
                <h3>📦</h3>
                <p>Pick Orders</p>
            </div>
            <div class="stat-card">
                <h3>✅</h3>
                <p>Mark as Served</p>
            </div>
        </div>

        <?php if ($ready_count > 0): ?>
            <div class="orders-grid">
                <?php while($order = $orders_result->fetch_assoc()): ?>
                    <div class="order-card <?php echo strtotime($order['created_at']) < time() - 300 ? 'urgent' : ''; ?>">
                        <div class="order-header">
                            <span class="order-id">#<?php echo $order['order_id']; ?></span>
                            <span class="table-no">Table <?php echo $order['table_id']; ?></span>
                        </div>
                        
                        <div class="order-items">
                            <h4>📋 Items Ready:</h4>
                            <div class="items-list">
                                <?php 
                                $types = array_filter(array_map('trim', explode(',', $order['types'])));
                                foreach($types as $type) {
                                    echo '<span class="item-type">' . htmlspecialchars($type) . '</span>';
                                }
                                ?>
                                <br>
                                <?php echo htmlspecialchars($order['items']); ?>
                            </div>
                        </div>
                        
                        <div class="order-meta">
                            <div class="meta-item">
                                <div class="meta-label">Amount Due</div>
                                <div class="meta-value">₹<?php echo number_format($order['total_amount'], 0); ?></div>
                            </div>
                            <div class="meta-item">
                                <div class="meta-label">Order Time</div>
                                <div class="meta-value"><?php echo date('H:i', strtotime($order['created_at'])); ?></div>
                            </div>
                        </div>
                        
                        <span class="status-badge">✅ Ready to Serve</span>
                        
                        <form method="POST" class="action-buttons">
                            <input type="hidden" name="order_id" value="<?php echo $order['order_id']; ?>">
                            <button type="submit" class="btn">🎉 Served</button>
                        </form>
                    </div>
                <?php endwhile; ?>
            </div>
        <?php else: ?>
            <div class="empty-state">
                <h2>✨ All Delivered!</h2>
                <p>No pending orders to serve</p>
            </div>
        <?php endif; ?>
    </div>

    <script>
        setInterval(function() {
            location.reload();
        }, 20000); // Auto-refresh every 20 seconds
    </script>
    
    <!-- Notification System -->
    <script src="../js/notifications.js"></script>
</body>
</html>
